...
class GoalCompositeAll extends Goal {
  ...
  heuristic (node) {
    // Number.MIN_VALUE is the smallest positive double (~5e-324), not
    // -Infinity; it was presumably meant to mirror GoalCompositeAny's
    // Number.MAX_VALUE sentinel below, but that symmetry doesn't hold in JS.
    // -Infinity is the correct identity element for a max-reduction: it's a
    // no-op against any real heuristic value, including a legitimately
    // negative one from a nested GoalInvert (Math.max(0, -5) would wrongly
    // clamp to 0; Math.max(-Infinity, -5) correctly stays -5).
    let max = -Infinity
    for (const i in this.goals) {
