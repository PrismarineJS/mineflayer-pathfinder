...
class GoalBreak extends Goal {
  constructor (x, y, z, bot, options = {}) {
    ...
    this.reach = options.reach || 4
    // this.RaycastIterator = require('prismarine-world').iterators.RaycastIterator
  }
  ...
  isEnd (node) {
    if (distance(node.x - this.x, node.y - this.y + this.bot.entity.height - 1, node.z - this.z) > this.reach) return false
    ...
    for (const i in visibleFaces) {
      ...
      // const rayIterator = new this.RaycastIterator(startPos, targetPos.clone().subtract(startPos).normalize(), 5) // overshoot a bit for more reliable raycasting
      const rayBlock = this.world.raycast(startPos, targetPos.clone().subtract(startPos).normalize(), this.reach + 1)
      if (rayBlock && rayBlock.x === this.x && rayBlock.y === this.y && rayBlock.z === this.z) {
        validFaces.push({
          face: rayBlock.face,
          targetPos
        })
    ...
    if (validFaces.length === 0) return false
    // Chose closest valid face
    let closest
    let distSqrt = 999
    for (const i in validFaces) {
      const tPos = validFaces[i].targetPos
      const cDist = new Vec3(tPos.x, tPos.y, tPos.z).distanceSquared(this.bot.entity.position.offset(0, this.bot.entity.height, 0))
      if (distSqrt > cDist) {
        closest = validFaces[i]
        distSqrt = cDist
      }
    }
    this.bot.lookAt(closest.targetPos, true)
    return true
  }
}
...
function simpleDistance (dx, dy, dz) {
  return Math.min(Math.abs(dx), Math.abs(dy), Math.abs(dz))
}

function distanceSquared (dx, dy, dz) {
