 ...
 */
const goto = (bot, goal, cb) => {
  const goalReachedListener = () => cleanup()

  const noPathListener = results => {
    if (results.status !== 'noPath') return
    cleanup(error('NoPath', 'No path to the goal!'))
  }

  const goalChangedListener = newGoal => {
    if (newGoal === goal) return
    cleanup(
      error('GoalChanged', 'The goal was changed before it could be completed!')
    )
  }

  const cleanup = err => {
    bot.removeListener('goal_reached', goalReachedListener)
  ...
  bot.on('goal_reached', goalReachedListener)
