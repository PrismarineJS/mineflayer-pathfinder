...
class ModularMovementsHandler {
    ...
    for (const plugin of this.movementPlugins) {
      for (const movement of plugin.movements) {
        if (movement.symmetry === "radial") {
          for (const del of radialSymmetry(movement.delta)) {
            const m = this._handleMovement(movement, del, node, world);
            if (m) neighbors.push(m);
          }
        } else {
          const m = this._handleMovement(movement, movement.delta, node, world);
          if (m) neighbors.push(m);
        }
      }
    }
   ...
   * @param {*} world - The virtual world state.
   */
  _handleMovement(movement, delta, node, world) {
    const position = deltaToVec(delta) + node.position;
    ...
    for (const action of movement.actions) {
      ...
      const action = new Action(type, block.position);

      if (type === "mine") {
        if (!this._canMine(block)) continue;

        action.tool = "hand"; // TODO Determine best tool
      }

      if (type === "build") {
        if (!this._canPlace(block)) continue;

        action.scaffolding = "dirt"; // TODO Determine correct scaffolding
      }
