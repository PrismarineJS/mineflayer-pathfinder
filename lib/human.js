...
function createHuman (bot, opts = {}) {
  ...
  function finishWalk (w) {
    ...
    if (w.faceAt) {
      ...
      settled(400).then(ok => {
        releaseHead()
        if (ok) w.resolve()
        else w.reject(new Error('head did not settle'))
      })
    } else {
      releaseHead()
      w.resolve()
  ...
  // Resolves true once the head has stopped moving for `ms`. The head only moves on a physics tick, so
  // the wait is also capped on wall clock: a kick, an unloaded chunk or physicsEnabled = false ends the
  // tick stream, and a caller awaiting the head would otherwise never be answered at all. Hitting the
  // cap resolves false, since the head may never have reached its target.
  function settled (ms, cap = ms + 2000) {
    return new Promise(resolve => {
      ...
      const check = () => {
        ...
        if (Date.now() - quietSince >= ms) done(true)
      }
      const timer = setTimeout(done, cap, false)
  ...
  async function lookAt (point, o = {}) {
    ...
    const ok = await settled(o.settleMs ?? 300)
    releaseHead()
    if (!ok) throw new Error('head did not settle')
  }
