...
// A door's open state, from its block properties (boolean on 1.13+, 'true'/'false' on legacy metadata)
function isOpen (block) {
  const props = block.getProperties ? block.getProperties() : null
  return !!props && (props.open === true || props.open === 'true')
}
...
class Movements {
  constructor (bot) {
    ...
    // Wooden doors: two blocks tall, and (unlike a gate) still shaped when open, so
    // they get their own set — getMoveForward reads the open state off the block
    this.doors = new Set()
    registry.blocksArray.forEach(block => {
      const name = block.name.toLowerCase()
      if (name.endsWith('_door') && !name.includes('iron') && !name.includes('trapdoor')) {
        this.doors.add(block.id)
        this.openable.add(block.id)
      }
    })

    this.canOpenDoors = false // Open fence gates and wooden doors on the way through instead of pathing around or digging them
   ...
   */
  safeToBreak (block) {
    // Gates and doors are walked through, never broken (a dig-through would otherwise
    // out-cost the click on the work profiles, and the diagonal into a door frame clips)
    if (this.canOpenDoors && block.openable) return false
    if (!this.canDig) {
      return false
    }
  ...
  getMoveForward (node, dir, neighbors) {
    ...
    const doorC = this.canOpenDoors && blockC.door
    // The upper half of a door being opened is not an obstacle
    if (!(doorC && blockB.door)) {
      cost += this.safeOrBreak(blockB, toBreak)
      if (cost > 100) return
    }

    if (doorC) {
      // Doors: an open door still has a collision shape (the leaf, swung into the
      // frame), so the gate test below would click it shut — read the state instead
      if (!isOpen(blockC)) toPlace.push({ x: node.x + dir.x, y: node.y, z: node.z + dir.z, dx: 0, dy: 0, dz: 0, useOne: true })
      cost += this.getNumEntitiesAt(blockC.position, 0, 0, 0) * this.entityCost
    } else if (this.canOpenDoors && blockC.openable && blockC.shapes && blockC.shapes.length !== 0) {
      // Closed fence gate: use it on the way through
    ...
    } else if (this.canOpenDoors && blockC.openable) {
      // Open fence gate: no shape, but its boundingBox is still 'block', so `safe` is
      // false and safeOrBreak would have it broken. It's a doorway — walk
      cost += this.getNumEntitiesAt(blockC.position, 0, 0, 0) * this.entityCost
    } else {
