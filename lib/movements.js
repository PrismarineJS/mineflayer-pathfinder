...
class Movements {
  ...
  getBlock (pos, dx, dy, dz) {
    const b = pos ? this.bot.blockAt(new Vec3(pos.x + dx, pos.y + dy, pos.z + dz)) : null
    if (!b) {
      return {
        safe: false,
        physical: false,
  ...
  getMoveDropDown (node, dir, neighbors) {
    ...
    for (let i = 0; i < this.maxDropDown - 1; i++) {
      if (blockLand.physical) break
      blockLand = this.getBlock(node, dir.x, -2 - i, dir.z)
      // TODO: check the fall path is safe
    }
    ...
    neighbors.push({
      x: blockLand.position.x,
      y: blockLand.position.y + 1,
      z: blockLand.position.z,
      remainingBlocks: node.remainingBlocks - toPlace.length,
      cost: cost,
      toBreak: toBreak,
      toPlace: toPlace
