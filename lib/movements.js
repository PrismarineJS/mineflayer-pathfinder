...
const doorOpenMetadata = [
  1, // open top south left
  9, // open bottom south left

  5, // open top south right
  13, // open bottom south right

  17, // open top north left
  25, // open bottom north left

  21, // open top north right
  29, // open bottom north right

  33, // open top east left
  41, // open bottom east left

  37, // open top east right
  35, // open bottom east right

  49, // open top west left
  57, // open bottom west left

  53, // open top west right
  61 // open bottom west right
]
...
class Movements {
  ...
  getBlock (pos, dx, dy, dz) {
    ...
    // determine if the door is a trap door or an iron door, we don't need that.
    if (b.name.includes('door') && !b.name.includes('trap') && !b.name.includes('iron')) {
      b.safe = true
      b.diggable = true

      if (doorOpenMetadata.includes(b.metadata)) {
        b.opened = true
        b.openable = false
        b.physical = false
        b.boundingBox = 'empty'
      ...
      if (doorClosedMetadata.includes(b.metadata)) {
        b.opened = false
        b.openable = true
        b.physical = true
        b.boundingBox = 'block'
  ...
  getMoveForward (node, dir, neighbors) {
    ...
    // Open fence gates
    if (this.canOpenDoors && blockC.openable && !blockC.opened && blockC.shapes && blockC.shapes.length !== 0) {
      // We need a way to find out where should the bot look after opening the door, to avoid getting stuck
      const doorData = {
        x: node.x + dir.x,
        y: node.y,
        z: node.z + dir.z,
        ...
        dy: 0,
        dz: 0,
        // px: (node.x + dir.x) - judge,
        // py: node.y,
        // pz: (node.z + dir.z) - judge,
        useOne: true
      }

      toPlace.push(doorData) // Indicate that a block should be used on this block not placed
    } else {
