...
class Movements {
  constructor (bot) {
    ...
    // Java blocks carry a static minStateId; Bedrock uses runtime block-state palettes and exposes defaultState instead
    // (minStateId is undefined), so fall back to it - otherwise fromStateId yields empty shapes and fences/carpets/
    // emptyBlocks are never classified on Bedrock.
    registry.blocksArray.map(x => Block.fromStateId(x.minStateId ?? x.defaultState, 0)).forEach(block => {
