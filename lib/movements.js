...
class Movements {
  constructor (bot, mcData) {
    ...
    this.exclusionAreas = []
    this.exclusionAreaPower = 8

    this.maxDropDown = 4
  }

  /**
   * Returns true if block is in excluded area. False if not.
   * @param {import('mineflayer-pathfinder').SafeBlock} block SaveBlock block
   * @returns {boolean}
   */
  isExcluded (block) {
    if (this.exclusionAreas.length === 0) return false
    for (const a of this.exclusionAreas) {
  ...
  safeToBreak (block) {
    ...
    return block.type && !this.blocksCantBreak.has(block.type) && !this.isExcluded(block) && this.exclusionAreaPower > 50
  }

  safeOrBreak (block, toBreak) {
    let exclusionExtraCost = 0
    if (this.isExcluded(block)) exclusionExtraCost = this.exclusionAreaPower // Is excluded so can't move or break
    if (block.safe) return exclusionExtraCost
    ...
    return (1 + 3 * digTime / 1000) * this.digCost + exclusionExtraCost
  }
  ...
  getMoveParkourForward (node, dir, neighbors) {
    ...
    for (let d = 2; d <= maxD; d++) {
      ...
      if (ceilingClear && !this.isExcluded(blockC) && blockB.safe && blockC.safe && blockD.physical) {
        // Forward
        neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks, 1, [], [], true))
        break
      } else if (ceilingClear && !this.isExcluded(blockB) && blockB.safe && blockC.physical) {
        // Up
        if (blockA.safe) {
      ...
      } else if ((ceilingClear || d === 2) && blockB.safe && blockC.safe && blockD.safe && !this.isExcluded(blockD) && floorCleared) {
        // Down
