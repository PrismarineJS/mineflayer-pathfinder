...
class Movements {
  constructor(bot, mcData, exclusion_areas = []) {
    this.bot = bot

    this.exclusion_areas = exclusion_areas
  ...
  safeToBreak(block) {
    ...
    const insideExclusionArea = this.exclusion_areas.some((area) => { area.insideArea(block.position) })
    
    return insideExclusionArea && block.type && !this.blocksCantBreak.has(block.type)
  }
