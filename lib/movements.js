...
class Movements {
  constructor (bot, mcData) {
    ...
    this.entsToAvoid = new Set()
    this.passableEnts = new Set(require('./passableEnts.json'))
  ...
  updateCollisionIndex () {
    for (const ent of Object.values(this.bot.entities)) {
      ...
      const avoidedEnt = this.entsToAvoid.has(ent.name)
      if (avoidedEnt || !this.passableEnts.has(ent.name)) {
        ...
        for (let y = minY; y < maxY; y++) {
          for (let x = minX; x < maxX; x++) {
            for (let z = minZ; z < maxZ; z++) {
              this.entIntersections[`${x},${y},${z}`] = this.entIntersections[`${x},${y},${z}`] ?? 0
              this.entIntersections[`${x},${y},${z}`] += cost // More ents = more weight
            }
          }
        }
      }
  ...
  getNumEntitiesAt (pos, dx, dy, dz) {
    ...
    return this.entIntersections[`${x},${y},${z}`] ?? 0
  }
