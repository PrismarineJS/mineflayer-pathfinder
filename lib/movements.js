...
class Movements {
  constructor (bot, mcData) {
    ...
    mcData.blocksArray.map(x => Block.fromStateId(x.minStateId, 0)).forEach(block => {
      ...
      } else if (interactable.includes(block.name) && (block.name.toLowerCase().includes('door') || block.name.toLowerCase().includes('gate')) && !block.name.toLowerCase().includes('iron')) {
        this.openable.add(block.type)
      }
    })
