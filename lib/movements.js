...
class Movements {
  ...
  getNumEntitiesAt (pos, dx, dy, dz) {
    let numEntities = 0
    if (this.allowEntityDetection === false) return numEntities

    const y = pos.y + dy
    const x = pos.x + dx
    const z = pos.z + dz

    if ((y in this.entIntersections) && (x in this.entIntersections[y])) {
      numEntities = this.entIntersections[y][x][z] || numEntities
    }
    return numEntities
  }
  ...
  getMoveJumpUp (node, dir, neighbors) {
    ...
    if (!blockB.physical && blockC.physical) cost += this.getNumEntitiesAt(blockB.position, 0, 0, 0) * this.entityCost // add cost for entities in B
  ...
  getMoveForward (node, dir, neighbors) {
    ...
    if (!blockC.physical && blockD.physical) cost += this.getNumEntitiesAt(blockC.position, 0, 0, 0) * this.entityCost // add cost for entities in C
  ...
  getMoveUp (node, neighbors) {
    ...
    if (block2.physical && (this.getNumEntitiesAt(node, 0, 3, 0) > 0)) return // an entity will fall into the building area
  ...
  getMoveParkourForward (node, dir, neighbors) {
    ...
    for (let d = 2; d <= maxD; d++) {
      ...
      if (ceilingClear && blockB.safe && blockC.safe && blockD.physical) {
        cost += this.exclusionStep(blockB) + 1
        // Forward
        ...
        // Up
        if (blockA.safe) {
          cost += this.exclusionStep(blockA) + 1
        ...
        if (blockE.physical) {
          cost += this.exclusionStep(blockD) + 1
