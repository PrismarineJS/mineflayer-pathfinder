...
class Movements {
  constructor (bot, mcData) {
    ...
    this.replaceables.add(mcData.blocksByName.grass.id)
    this.replaceables.add(mcData.blocksByName.vine.id)
    this.replaceables.add(mcData.blocksByName.water.id)
    this.replaceables.add(mcData.blocksByName.lava.id)
    this.replaceables.add(mcData.blocksByName.seagrass.id)
    this.replaceables.add(mcData.blocksByName.tall_seagrass.id)
    this.replaceables.add(mcData.blocksByName.snow.id) // only if !physical
  ...
  getBlock (pos, dx, dy, dz) {
    ...
    b.safe = (b.boundingBox === 'empty' || b.climbable) && !this.blocksToAvoid.has(b.type)
  ...
  getMoveDiagonal (node, dir, neighbors) {
    ...
    if (!blockD.physical && !blockC.liquid) return // we don't place blocks in diagonal
  ...
  getMoveUp (node, neighbors) {
    ...
    if (!block1.climbable) {
      ...
      const block0 = this.getBlock(node, 0, -1, 0)
      if (block0.physical && block0.height < node.y) return // cannot jump-place from a half block
