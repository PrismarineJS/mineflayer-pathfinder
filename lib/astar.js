...
class AStar {
  constructor(start, movements, goal, timeout) {
    ...
    const startNode = this.bestNode = new PathNode().set(start, 0, goal.heuristic(start))
  ...
  compute() {
    ...
    while (!this.openHeap.isEmpty()) {
      if (performance.now() - computeStartTime > this.timeout) {
