...
class AStar {
  constructor (start, movements, goal, timeout, tickTimeout = 40, searchRadius = -1, options = {}) {
    ...
    this.maxVisitedNodes = options.maxVisitedNodes ?? -1
    this.nodeEvaluator = typeof options.nodeEvaluator === 'function' ? options.nodeEvaluator : null
    ...
    this.bestNodeScore = this.evaluateNode(startNode)
  ...
  evaluateNode (node) {
    if (!this.nodeEvaluator) return null
    const score = Number(this.nodeEvaluator(node.data, node.g))
    return Number.isFinite(score) ? score : -Infinity
  }

  considerBestNode (node) {
    if (!this.nodeEvaluator) {
      if (node.h < this.bestNode.h) this.bestNode = node
      return
    }
    const score = this.evaluateNode(node)
    if (score > this.bestNodeScore || (score === this.bestNodeScore && node.g < this.bestNode.g)) {
      this.bestNode = node
      this.bestNodeScore = score
    }
  }
  ...
  makeResult (status, node) {
    return {
      ...
      objectiveScore: this.nodeEvaluator ? this.evaluateNode(node) : null,
      context: this
    }
  }
  ...
  compute () {
    ...
    while (!this.openHeap.isEmpty()) {
      ...
      if (this.maxVisitedNodes >= 0 && this.closedDataSet.size >= this.maxVisitedNodes) {
        return this.makeResult('budget', this.bestNode)
      }
      ...
      for (const neighborData of neighbors) {
        ...
        this.considerBestNode(neighborNode)
        if (update) {
