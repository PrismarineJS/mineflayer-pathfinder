...
export class PathFinder extends EventEmitter {
  ...
  getPositionOntopOf (block) {
    ...
    return block.position.offset(x / n, y, z / n)
  }
  ...
  async monitorMovement () {
    ...
    if (!goal) return
    if (goal instanceof goals.GoalFollow && this.movements.allowFreeMotion) {
    ...
    if (nextPoint.toBreak.length > 0 && bot.entity.onGround) {
      ...
      if (!block) {
        bot.emit(
          'warn',
          `Attempted to get a block at ${point.toString()}, but couldn't`
        )
        return
      }
    ...
    if (nextPoint.toPlace.length > 0) {
      ...
      if (canPlace) {
        ...
        return new Promise((resolve, reject) =>
          bot.placeBlock(reference, facing, error => {
            this.lastNodeTime = performance.now()
            this.placing = false
            if (error) {
              this.resetPath()
    ...
    const _nextPoint =
      this.getPositionOntopOf(
        bot.blockAt(new Vec3(nextPoint.x, nextPoint.y, nextPoint.z))
      ) ||
      this.getPositionOntopOf(
        bot.blockAt(new Vec3(nextPoint.x, nextPoint.y - 1, nextPoint.z))
      )
    if (_nextPoint) {
    ...
    const distanceXZTotal = dx * dx + dz * dz
    if (
      distanceXZTotal <= POINT_15_SQ &&
      (bot.entity.onGround || bot.entity.isInWater)
    ) {
    ...
    let gottaJump = false
    const horizontalDelta = Math.sqrt(distanceXZTotal)
  ...
  _blockUpdateListener (oldBlock, newBlock) {
    try {
      if (
        ...
        !this.isPositionNearPath(oldBlock.position)
      ) {
        return
      }
